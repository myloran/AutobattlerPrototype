...
namespace PlasticFloor.EventBus {
  ...
  public class EventBus : IEventBus {
    ...
    readonly List<IHandlerProvider> _handlerProviders = new List<IHandlerProvider>();
    ...
    public void Register<TEvent>(params IEventHandler<TEvent>[] handlers) where TEvent : IEvent {
      if (!_handlers.ContainsKey(typeof(TEvent))) 
        _handlers.Add(typeof(TEvent), new List<Delegate>());
      
      foreach (var handler in handlers)
        _handlers[typeof(TEvent)].Add(new EventHandlerDelegate<TEvent>(handler.HandleEvent));
    }
    ...
    void ExecuteHandler<TEvent>(EventHandlerDelegate<TEvent> handler, TEvent @event, bool safe) where TEvent : IEvent {
      if (safe)
        try {
    ...
    void Handle<TEvent>(TEvent @event, bool safe) where TEvent : IEvent {
